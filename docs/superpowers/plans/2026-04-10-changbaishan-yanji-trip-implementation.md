...
**Goal:** Refresh `吉林延吉长白山trip.html` with a cleaner premium travel-editorial look while preserving content order, create `吉林延吉长白山trip-专题版.html` with an answer-first reading flow, and add recent transport schedule/price guidance plus writing rules in `travel-planning-playbook.md`.

**Architecture:** Keep both deliverables as standalone offline HTML files with no external dependencies. Reuse one visual system across both pages, preserve anchors and section order in the original file, and only reorganize/dedupe content in the new专题版 copy. Add transport tables that separate recent checked schedules from estimated pricing, and update the playbook so future travel pages always include recent-query dates, transport modes, and fallback wording when holiday inventory is not yet open. Add one lightweight PowerShell regression script so structure checks are repeatable.

**Tech Stack:** HTML, CSS, minimal vanilla JS only if required, PowerShell validation script, Git, recent web research from official/primary travel sources
...
### Task 4: Add Transport Research Blocks And Update The Playbook
...
- [ ] **Step 1: Extend the regression script with failing transport-content checks**

Append checks for transport markers and recent-query wording:
...
Expected: FAIL on missing transport wording or playbook markers.

- [ ] **Step 3: Add recent transport schedules and estimated pricing to both HTML files**

Use the most recent queryable transport data and clearly label the query date. For holiday inventory not yet open, state that the listed services are the latest queryable reference and that final sale status may change.

Required content blocks for both HTML files:

```html
<article class="feature-panel">
  <div class="section-intro">
    <p class="kicker">交通班次与价格参考</p>
    <h3>截至 2026 年 4 月 10 日最近可查</h3>
    <p>五一正式售卖库存可能后续调整，下面先用最近可查班次做执行参考。公共交通线路通常稳定，节假日若有变化一般会另发公告。</p>
  </div>
</article>
```

Within each recommended route or plan, cover these three layers:

```html
<ul>
  <li><strong>高铁 / 动车：</strong>列出最近可查车次、出发到达时间、二等座参考价。</li>
  <li><strong>飞机：</strong>列出直飞或更稳妥的中转时刻、飞行时长、最近可查票价区间。</li>
  <li><strong>公交 / 地铁 / 打车：</strong>列出进出机场 / 火车站 / 市区的主力线路、运行时段、票价或打车估算。</li>
</ul>
```
...
- [ ] **Step 4: Update the playbook with transport-writing rules**

Add a dedicated section to `travel-planning-playbook.md` that future pages must follow:

```markdown
## 交通信息写法

- 每个推荐方案都要写清高铁 / 动车、飞机、公交 / 地铁 / 打车这几类交通选择。
- 交通信息必须标注最近可查日期，例如：`截至 2026 年 4 月 10 日最近可查`。
- 如果节假日正式班次还未完全放票，要明确说明“以下为最近可查参考班次，最终以出发日前官方平台为准”。
- 所有交通方案都要给预估价格，哪怕只能给区间，也不要只给“自行查询”。
- 城市内接驳优先写：机场 / 高铁站到核心住宿区、热门商圈、景区集散点。
```

- [ ] **Step 5: Run the full regression script and commit the transport-content update**
...
```text
transport content checks passed
```

Commit:

```bash
git add tests/trip-page-regression.ps1 "吉林延吉长白山trip.html" "吉林延吉长白山trip-专题版.html" "travel-planning-playbook.md"
git commit -m "feat: add transport guidance to trip pages and playbook"
```
...
- 最近可查交通班次、价格估算和 playbook 写法更新：Task 4
